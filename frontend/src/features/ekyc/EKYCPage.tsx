...
import type { ReviewKYCDto, SubmitKYCDto } from '../../api/kyc'
...
export default function EKYCPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      kycApi.list({ page, pageSize: PAGE_SIZE, status: statusFilter || undefined }).then(r => r.data),
  })

  const { data: customersData } = useQuery({
    queryKey: ['customers-all'],
    queryFn: () => customersApi.list({ page: 1, pageSize: 200 }).then(r => r.data),
    enabled: submitOpen,
  })

  const records: KYCVerification[] = data?.data ?? []
  const customers = customersData?.data ?? []
  const totalPages = data?.meta ? Math.ceil(data.meta.total / data.meta.pageSize) : 1

  function invalidate() { queryClient.invalidateQueries({ queryKey: ['kyc'] }) }

  const submitMutation = useMutation({
    mutationFn: (dto: SubmitKYCDto) => kycApi.submit(dto),
    onSuccess: () => {
      invalidate()
      setSubmitOpen(false)
      setSubmitCustomerId('')
  ...
  async function handleKYCSubmit() {
    if (!submitCustomerId) return
    setSubmitUploading(true)
    try {
      const [idDocumentUrl, selfieUrl] = await Promise.all([
        idDocFile ? uploadFile(idDocFile) : Promise.resolve(undefined),
        selfieFile ? uploadFile(selfieFile) : Promise.resolve(undefined),
      ])
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, dto }: { id: string; dto: ReviewKYCDto }) => kycApi.approve(id, dto),
    onSuccess: () => { invalidate(); setActionModal(closedModal); setNotes('') },
  })

  const rejectMutation = useMutation({
    mutationFn: ({ id, dto }: { id: string; dto: ReviewKYCDto }) => kycApi.reject(id, dto),
    onSuccess: () => { invalidate(); setActionModal(closedModal); setNotes('') },
  })

  const setInReviewMutation = useMutation({
    mutationFn: ({ id, notes }: { id: string; notes?: string }) => kycApi.setInReview(id, { notes }),
    onSuccess: () => { invalidate(); setActionModal(closedModal); setNotes('') },
  })

  const requestDocsMutation = useMutation({
    mutationFn: ({ id, notes }: { id: string; notes: string }) => kycApi.requestDocs(id, { notes }),
    onSuccess: () => { invalidate(); setActionModal(closedModal); setNotes('') },
  })

  function openAction(record: KYCVerification, action: ModalAction) {
    setNotes('')
  ...
  const columns: Column<KYCVerification>[] = [
    {
      key: 'customerName',
      header: 'Customer',
      render: (row) => <span className="font-medium text-slate-800">{row.customerName ?? '—'}</span>,
    },
    {
      key: 'submittedBy',
      header: 'Submitted By',
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1.5 flex-wrap">
          <ActionBtn color="slate" onClick={() => navigate(`/ekyc/${row.id}`)}>View</ActionBtn>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">eKYC Verifications</h1>
          <p className="text-sm text-slate-500 mt-0.5">Customer identity verification requests.</p>
        </div>
        <Button variant="primary" onClick={() => setSubmitOpen(true)}>+ Submit KYC</Button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <Table columns={columns} data={records} isLoading={isLoading} emptyText="No verification requests found." />
      ...
      {/* Submit modal */}
      <Modal open={submitOpen} onClose={() => { if (!submitMutation.isPending && !submitUploading) setSubmitOpen(false) }} title="Submit KYC Verification" maxWidth="max-w-lg">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label htmlFor="kyc-customer" className="text-sm font-medium text-slate-700">
            ...
            >
              ...
              {customers.map((c) => (
                <option key={c.id} value={c.id}>{c.fullName} ({c.idNumber})</option>
              ))}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <FileField
              id="kyc-id-doc"
              label="ID Document (KTP/Passport)"
              file={idDocFile}
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="kyc-sub-notes" className="text-sm font-medium text-slate-700">
              Notes <span className="text-slate-400 font-normal">(optional)</span>
            </label>
            <textarea id="kyc-sub-notes" rows={2} value={submitNotes} onChange={(e) => setSubmitNotes(e.target.value)}
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
            ...
            <Button variant="primary" loading={submitMutation.isPending || submitUploading} onClick={handleKYCSubmit} disabled={!submitCustomerId}>
      ...
      {/* Action modal (approve / reject / in_review / request_docs) */}
      {cfg && (
          ...
          <div className="flex flex-col gap-4">
            <p className="text-sm text-slate-600">
              {actionModal.action === 'in_review'
                ...
                ? 'Approve eKYC verification for '
                : 'Reject eKYC verification for '}
              <span className="font-semibold text-slate-800">
                {actionModal.record?.customerName ?? actionModal.record?.customerId}
              </span>.
            </p>
            <div className="flex flex-col gap-1">
              ...
              <textarea id="action-notes" rows={4} value={notes} onChange={(e) => setNotes(e.target.value)}
...
function FileField({ id, label, file, onChange, disabled }: FileFieldProps) {
  return (
    <div className="flex flex-col gap-1">
      <label htmlFor={id} className="text-sm font-medium text-slate-700">{label}</label>
      <label htmlFor={id}
        className={[
          'flex flex-col items-center justify-center gap-1 px-3 py-3 rounded-lg border-2 border-dashed cursor-pointer transition-colors text-center',
          disabled ? 'border-slate-200 bg-slate-50 cursor-not-allowed' : 'border-slate-300 hover:border-sky-400 hover:bg-sky-50',
        ].join(' ')}>
        {file
          ? <span className="text-xs text-sky-700 font-medium truncate max-w-full px-1">{file.name}</span>
          : <span className="text-xs text-slate-400">Click to upload<br/><span className="text-slate-300">JPG, PNG, PDF, WEBP · max 10 MB</span></span>}
        <input
          id={id}
          type="file"
